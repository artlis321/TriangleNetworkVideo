...
def testO(A,B,C,O):
    ...
    A0O = 1
    A1B0 = np.sin(np.pi - ang_A - ang_B) / np.sin(ang_B)
    B0O = np.sin(ang_A) / np.sin(ang_B)
    A1C0 = np.sin(np.pi - ang_A - ang_C) / np.sin(ang_C)
    C0O = np.sin(ang_A) / np.sin(ang_C)
    B0C0 = np.sin(2*ang_A) * A1B0 / np.sin(2*ang_C)

    IP = A1B0 + A1C0 + B0C0 # perimeter of inner triangle (added lines)
    II = A0O + B0O + C0O # inner segments of inner triangle (removed lines)

    AO = d(A,O)
    BO = d(B,O)
    CO = d(C,O)

    scale_max = min([AO/A0O,BO/B0O,CO/C0O])
